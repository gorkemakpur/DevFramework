        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (token != null)
                {
                    ...
                    IUserService userService = InstanceFactory.GetInstance<IUserService>(); //busines module içinden bir çözüm üret 
                    var user = userService.GetByUserNameAndPassword(tokenValues[0], tokenValues[1]);

                    if (tokenValues[0] == "aziz" && tokenValues[1] == "123")
                    {
                        ...
                        HttpContext.Current.User = principal; //web apide authorize ı rolleriyle kullanmak için
                    }
                }
            }
            catch
